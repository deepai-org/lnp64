...
Field layout (one 8-byte word): `opcode[63:56]`, `rd[55:51]`, `rs1[50:46]`,
`rs2[45:41]`, `rs3[40:36]`, `rs4[35:31]`, `rs5[30:26]`; the 32-bit immediate
sits below the lowest register slot the format uses (I-type `[45:14]`, S/B-type
`[40:9]`, U/J-type `[50:19]`). Branch and jump offsets are instruction-counts:
`target = PC + (sext32(imm) << 3)`. The full opcode/format table is
`isa_v2_opcodes.md`. The legacy v1 32-bit encoding and the earlier
variable-width 64-bit sketch that previously occupied this section have been
removed.
