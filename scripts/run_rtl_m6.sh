...
rtl_binary="$build_dir/Vlnp64_m6_tb"
seeds="${LNP64_COSIM_SEEDS:-0}"

rtl_verilator_build_or_reuse \
  "$build_dir" \
  "$rtl_binary" \
  "/tmp/lnp64_rtl_m6_build.log" \
  "${common_flags[@]}" \
  "${rtl_files[@]}"

rtl_run_seeded_trace_cosim "m6" "$rtl_binary" "formal/m6_service_model.py" "LNP64-RTL-M6 PASS" "$seeds"
