#!/usr/bin/env python3
"""Check the M4 RTL typed VMA/MMU commit trace.

Follows the M1/M7 typed-trace pattern for the SG-MEM slice. The lnp64_m4_vma
engine emits a schema-owned packed commit (lnp64_m4_vma_commit_t) and state
projection (lnp64_m4_state_projection_t) per VMA transition: mmap, permitted
...
DEFAULT_M4_TRACE_LOG = Path("/tmp/lnp64_rtl_m4_typed_commit.log")

COMMIT_NAME = "m4_vma_commit"
STATE_NAME = "m4_state_projection"
COMMIT_BITS_NAME = "m4_vma_commit_bits"
STATE_BITS_NAME = "m4_state_projection_bits"
COMMIT_RECORD = "lnp64_m4_vma_commit_t"
STATE_RECORD = "lnp64_m4_state_projection_t"
OP_ENUM = "lnp64_m4_vma_op_e"

WITNESS_SCHEMA = "lnp64_m4_vma_refinement_witness_v1"
...
def fail(message: str) -> None:
    raise SystemExit(f"M4 typed commit check failed: {message}")
...
def run_m4_gate() -> str:
    log_path = Path(os.environ.get("LNP64_M4_TYPED_COMMIT_LOG", DEFAULT_M4_TRACE_LOG))
    if os.environ.get("LNP64_M4_TYPED_COMMIT_USE_EXISTING") == "1":
        try:
            return log_path.read_text(encoding="utf-8")
        except OSError as exc:
            fail(f"missing existing M4 typed commit log {log_path}: {exc}")
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        fail(f"scripts/run_rtl_m4.sh exited with {proc.returncode}")
    try:
...
def expected_sequence(ops: dict[str, int]) -> list[int]:
    return [
        ops["LNP64_M4_COMMIT_MMAP"],
        ops["LNP64_M4_COMMIT_LOAD"],
        ops["LNP64_M4_COMMIT_STORE_DENIED"],
        ops["LNP64_M4_COMMIT_EXEC_FAULT"],
        ops["LNP64_M4_COMMIT_GUARD_FAULT"],
        ops["LNP64_M4_COMMIT_STALE_REJECT"],
        ops["LNP64_M4_COMMIT_TLB_INVALIDATE"],
    ]
...
def check_transition_trace(commits: list[dict], states: list[dict], ops: dict[str, int]) -> None:
    ...
    if actual != expected_sequence(ops):
        fail(f"M4 typed commit sequence drifted: {actual} != {expected_sequence(ops)}")
    by_op = {v: k for k, v in ops.items()}
    for index, (commit, state) in enumerate(zip(commits, states, strict=True)):
        op = require_int(commit, "op")
        # The W^X invariant must hold in every committed projection.
        if require_int(state, "wx_enforced") != 1:
            fail(f"M4 commit {index} ({by_op.get(op)}) violated the W^X invariant")
        if require_int(commit, "op") != require_int(state, "op") or require_int(commit, "status") != require_int(state, "status"):
            fail(f"M4 commit {index} op/status drifted from state projection")
        if op == ops["LNP64_M4_COMMIT_MMAP"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "mapping_created") != 1:
                fail(f"M4 mmap commit {index} did not create a mapping")
        elif op == ops["LNP64_M4_COMMIT_LOAD"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "load_permitted") != 1:
                fail(f"M4 load commit {index} did not permit the load")
        elif op == ops["LNP64_M4_COMMIT_STORE_DENIED"]:
            if require_int(commit, "status") != ERR_EACCES or require_int(state, "store_rejected") != 1:
                fail(f"M4 store-denied commit {index} did not reject the write")
        elif op == ops["LNP64_M4_COMMIT_EXEC_FAULT"]:
            if require_int(commit, "status") != ERR_EFAULT or require_int(state, "nx_faulted") != 1:
                fail(f"M4 exec-fault commit {index} did not raise the NX fault")
        elif op == ops["LNP64_M4_COMMIT_GUARD_FAULT"]:
            if require_int(commit, "status") != ERR_EFAULT or require_int(state, "guard_faulted") != 1:
                fail(f"M4 guard-fault commit {index} did not raise the guard fault")
        elif op == ops["LNP64_M4_COMMIT_STALE_REJECT"]:
            if require_int(commit, "status") != ERR_EREVOKED or require_int(state, "stale_vma_rejected") != 1:
                fail(f"M4 stale-reject commit {index} did not reject the stale generation")
        elif op == ops["LNP64_M4_COMMIT_TLB_INVALIDATE"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "tlb_invalidation_observed") != 1:
                fail(f"M4 tlb-invalidate commit {index} did not observe the invalidation")
        else:
            fail(f"M4 commit {index} has unknown op {op}")
...
def main() -> int:
    ...
    output = run_m4_gate()
    commits = parse_json_records(output, "TTRACE_M4 ", COMMIT_NAME, commit_fields)
    commit_bits = parse_bit_records(output, "TTRACE_M4_BITS ", COMMIT_BITS_NAME, sum(commit_widths))
    states = parse_json_records(output, "TTRACE_M4_STATE ", STATE_NAME, state_fields)
    state_bits = parse_bit_records(output, "TTRACE_M4_STATE_BITS ", STATE_BITS_NAME, sum(state_widths))
    check_bits(commits, commit_bits, commit_fields, commit_widths, "M4 typed commit")
    check_bits(states, state_bits, state_fields, state_widths, "M4 state projection")
    if len(commits) != len(states):
        fail(f"M4 commit count {len(commits)} != state projection count {len(states)}")
    check_transition_trace(commits, states, ops)

    witness_out = os.environ.get("LNP64_RTL_M4_WITNESS_OUT")
    if witness_out:
    ...
    print("rtl m4 typed commit trace ok")
    return 0
