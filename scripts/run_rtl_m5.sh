...
if ! command -v verilator >/dev/null 2>&1; then
  printf '%s\n' "verilator is required for the RTL M5 gate" >&2
  exit 1
fi
...
common_flags=(
  ...
  -Wno-WIDTH
  -Wno-SYNCASYNCNET
  --top-module lnp64_m5_tb
)

mapfile -t rtl_files < tests/rtl/m5_filelist.f

build_dir="${TMPDIR:-/tmp}/lnp64_rtl_m5_obj"
seeds="${LNP64_COSIM_SEEDS:-0}"
rm -rf "$build_dir"

verilator --lint-only "${common_flags[@]}" "${rtl_files[@]}"
verilator --binary --Mdir "$build_dir" "${common_flags[@]}" "${rtl_files[@]}" >/tmp/lnp64_rtl_m5_build.log
...
for seed in $seeds; do
  ...
  "$build_dir/Vlnp64_m5_tb" "+seed=$seed" | tee "$rtl_log"
