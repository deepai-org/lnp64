#!/usr/bin/env python3
"""Check the M2 RTL typed gate commit trace.

Follows the M1/M7 typed-trace pattern for the SG-MEM slice. The lnp64_m2_gate
engine emits a schema-owned packed commit (lnp64_m2_gate_commit_t) and state
projection (lnp64_m2_state_projection_t) per VMA transition: mmap, permitted
...
DEFAULT_M2_TRACE_LOG = Path("/tmp/lnp64_rtl_m2_typed_commit.log")

COMMIT_NAME = "m2_gate_commit"
STATE_NAME = "m2_state_projection"
COMMIT_BITS_NAME = "m2_gate_commit_bits"
STATE_BITS_NAME = "m2_state_projection_bits"
COMMIT_RECORD = "lnp64_m2_gate_commit_t"
STATE_RECORD = "lnp64_m2_state_projection_t"
OP_ENUM = "lnp64_m2_gate_op_e"

WITNESS_SCHEMA = "lnp64_m2_gate_refinement_witness_v1"
...
def fail(message: str) -> None:
    raise SystemExit(f"M2 typed commit check failed: {message}")
...
def run_m2_gate() -> str:
    log_path = Path(os.environ.get("LNP64_M2_TYPED_COMMIT_LOG", DEFAULT_M2_TRACE_LOG))
    if os.environ.get("LNP64_M2_TYPED_COMMIT_USE_EXISTING") == "1":
        try:
            return log_path.read_text(encoding="utf-8")
        except OSError as exc:
            fail(f"missing existing M2 typed commit log {log_path}: {exc}")
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        fail(f"scripts/run_rtl_m2.sh exited with {proc.returncode}")
    try:
...
def expected_sequence(ops: dict[str, int]) -> list[int]:
    return [
        ops["LNP64_M2_COMMIT_SYNC_CALL"],
        ops["LNP64_M2_COMMIT_SYNC_RETURN"],
        ops["LNP64_M2_COMMIT_ASYNC_CALL"],
        ops["LNP64_M2_COMMIT_HANDOFF_CALL"],
        ops["LNP64_M2_COMMIT_STALE_RETURN"],
        ops["LNP64_M2_COMMIT_FAULT_DELIVERY"],
        ops["LNP64_M2_COMMIT_SIGNAL_COMPAT"],
    ]
...
def check_transition_trace(commits: list[dict], states: list[dict], ops: dict[str, int]) -> None:
    ...
    if actual != expected_sequence(ops):
        fail(f"M2 typed commit sequence drifted: {actual} != {expected_sequence(ops)}")
    by_op = {v: k for k, v in ops.items()}
    for index, (commit, state) in enumerate(zip(commits, states, strict=True)):
        op = require_int(commit, "op")
        # Continuation uniqueness must hold in every committed projection: a
        # second continuation can never be installed over a live one.
        if require_int(state, "continuation_unique") != 1:
            fail(f"M2 commit {index} ({by_op.get(op)}) violated continuation uniqueness")
        if require_int(commit, "op") != require_int(state, "op") or require_int(commit, "status") != require_int(state, "status"):
            fail(f"M2 commit {index} op/status drifted from state projection")
        if op == ops["LNP64_M2_COMMIT_SYNC_CALL"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "continuation_valid") != 1:
                fail(f"M2 sync-call commit {index} did not install a continuation")
        elif op == ops["LNP64_M2_COMMIT_SYNC_RETURN"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "sync_roundtrip_ok") != 1:
                fail(f"M2 sync-return commit {index} did not complete the roundtrip")
            # A sync return must consume (invalidate) its continuation.
            if require_int(state, "continuation_valid") != 0:
                fail(f"M2 sync-return commit {index} left a live continuation")
        elif op == ops["LNP64_M2_COMMIT_ASYNC_CALL"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "async_delivery_ok") != 1:
                fail(f"M2 async-call commit {index} did not deliver")
        elif op == ops["LNP64_M2_COMMIT_HANDOFF_CALL"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "handoff_delivery_ok") != 1:
                fail(f"M2 handoff-call commit {index} did not deliver")
        elif op == ops["LNP64_M2_COMMIT_STALE_RETURN"]:
            if require_int(commit, "status") != ERR_EREVOKED or require_int(state, "stale_continuation_rejected") != 1:
                fail(f"M2 stale-return commit {index} did not reject the stale continuation")
        elif op == ops["LNP64_M2_COMMIT_FAULT_DELIVERY"]:
            if require_int(commit, "status") != ERR_EFAULT or require_int(state, "fault_delivery_gate_ok") != 1:
                fail(f"M2 fault-delivery commit {index} did not deliver through the fault gate")
        elif op == ops["LNP64_M2_COMMIT_SIGNAL_COMPAT"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "signal_compatibility_ok") != 1:
                fail(f"M2 signal-compat commit {index} did not run the compatibility path")
        else:
            fail(f"M2 commit {index} has unknown op {op}")
...
def main() -> int:
    ...
    output = run_m2_gate()
    commits = parse_json_records(output, "TTRACE_M2 ", COMMIT_NAME, commit_fields)
    commit_bits = parse_bit_records(output, "TTRACE_M2_BITS ", COMMIT_BITS_NAME, sum(commit_widths))
    states = parse_json_records(output, "TTRACE_M2_STATE ", STATE_NAME, state_fields)
    state_bits = parse_bit_records(output, "TTRACE_M2_STATE_BITS ", STATE_BITS_NAME, sum(state_widths))
    check_bits(commits, commit_bits, commit_fields, commit_widths, "M2 typed commit")
    check_bits(states, state_bits, state_fields, state_widths, "M2 state projection")
    if len(commits) != len(states):
        fail(f"M2 commit count {len(commits)} != state projection count {len(states)}")
    check_transition_trace(commits, states, ops)

    witness_out = os.environ.get("LNP64_RTL_M2_WITNESS_OUT")
    if witness_out:
    ...
    print("rtl m2 typed commit trace ok")
    return 0
