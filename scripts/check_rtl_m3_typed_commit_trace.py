#!/usr/bin/env python3
"""Check the M3 RTL typed process commit trace.

Follows the M1/M7 typed-trace pattern for the SG-MEM slice. The lnp64_m3_process
engine emits a schema-owned packed commit (lnp64_m3_process_commit_t) and state
projection (lnp64_m3_state_projection_t) per VMA transition: mmap, permitted
...
DEFAULT_M3_TRACE_LOG = Path("/tmp/lnp64_rtl_m3_typed_commit.log")

COMMIT_NAME = "m3_process_commit"
STATE_NAME = "m3_state_projection"
COMMIT_BITS_NAME = "m3_process_commit_bits"
STATE_BITS_NAME = "m3_state_projection_bits"
COMMIT_RECORD = "lnp64_m3_process_commit_t"
STATE_RECORD = "lnp64_m3_state_projection_t"
OP_ENUM = "lnp64_m3_process_op_e"

WITNESS_SCHEMA = "lnp64_m3_process_refinement_witness_v1"
...
def fail(message: str) -> None:
    raise SystemExit(f"M3 typed commit check failed: {message}")
...
def run_m3_gate() -> str:
    log_path = Path(os.environ.get("LNP64_M3_TYPED_COMMIT_LOG", DEFAULT_M3_TRACE_LOG))
    if os.environ.get("LNP64_M3_TYPED_COMMIT_USE_EXISTING") == "1":
        try:
            return log_path.read_text(encoding="utf-8")
        except OSError as exc:
            fail(f"missing existing M3 typed commit log {log_path}: {exc}")
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        fail(f"scripts/run_rtl_m3.sh exited with {proc.returncode}")
    try:
...
def expected_sequence(ops: dict[str, int]) -> list[int]:
    return [
        ops["LNP64_M3_COMMIT_CLONE"],
        ops["LNP64_M3_COMMIT_CHILD_EXIT"],
        ops["LNP64_M3_COMMIT_PARENT_JOIN"],
        ops["LNP64_M3_COMMIT_EXEC_BARRIER"],
        ops["LNP64_M3_COMMIT_STALE_JOIN"],
        ops["LNP64_M3_COMMIT_EXEC_CANCEL"],
    ]
...
def check_transition_trace(commits: list[dict], states: list[dict], ops: dict[str, int]) -> None:
    ...
    if actual != expected_sequence(ops):
        fail(f"M3 typed commit sequence drifted: {actual} != {expected_sequence(ops)}")
    by_op = {v: k for k, v in ops.items()}
    for index, (commit, state) in enumerate(zip(commits, states, strict=True)):
        op = require_int(commit, "op")
        # Scheduler uniqueness for the process slice: exactly one thread
        # location must hold in every committed projection (SG-SCHED).
        if require_int(state, "exactly_one_thread_location") != 1:
            fail(f"M3 commit {index} ({by_op.get(op)}) violated exactly-one-thread-location")
        if require_int(commit, "op") != require_int(state, "op") or require_int(commit, "status") != require_int(state, "status"):
            fail(f"M3 commit {index} op/status drifted from state projection")
        if op == ops["LNP64_M3_COMMIT_CLONE"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "clone_created") != 1:
                fail(f"M3 clone commit {index} did not create the child")
        elif op == ops["LNP64_M3_COMMIT_CHILD_EXIT"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "child_exit_signaled") != 1:
                fail(f"M3 child-exit commit {index} did not signal the exit")
        elif op == ops["LNP64_M3_COMMIT_PARENT_JOIN"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "parent_join_completed") != 1:
                fail(f"M3 parent-join commit {index} did not complete the join")
        elif op == ops["LNP64_M3_COMMIT_EXEC_BARRIER"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "exec_barrier_stopped_sibling") != 1:
                fail(f"M3 exec-barrier commit {index} did not stop the sibling")
        elif op == ops["LNP64_M3_COMMIT_STALE_JOIN"]:
            if require_int(commit, "status") != ERR_EREVOKED or require_int(state, "stale_join_rejected") != 1:
                fail(f"M3 stale-join commit {index} did not reject the stale join")
        elif op == ops["LNP64_M3_COMMIT_EXEC_CANCEL"]:
            if require_int(commit, "status") != ERR_ECANCELED or require_int(state, "exec_cancel_terminal") != 1:
                fail(f"M3 exec-cancel commit {index} did not reach the terminal cancel")
        else:
            fail(f"M3 commit {index} has unknown op {op}")
...
def main() -> int:
    ...
    output = run_m3_gate()
    commits = parse_json_records(output, "TTRACE_M3 ", COMMIT_NAME, commit_fields)
    commit_bits = parse_bit_records(output, "TTRACE_M3_BITS ", COMMIT_BITS_NAME, sum(commit_widths))
    states = parse_json_records(output, "TTRACE_M3_STATE ", STATE_NAME, state_fields)
    state_bits = parse_bit_records(output, "TTRACE_M3_STATE_BITS ", STATE_BITS_NAME, sum(state_widths))
    check_bits(commits, commit_bits, commit_fields, commit_widths, "M3 typed commit")
    check_bits(states, state_bits, state_fields, state_widths, "M3 state projection")
    if len(commits) != len(states):
        fail(f"M3 commit count {len(commits)} != state projection count {len(states)}")
    check_transition_trace(commits, states, ops)

    witness_out = os.environ.get("LNP64_RTL_M3_WITNESS_OUT")
    if witness_out:
    ...
    print("rtl m3 typed commit trace ok")
    return 0
