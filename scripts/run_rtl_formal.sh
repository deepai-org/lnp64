...
if yosys-smtbmc --unroll -s "$solver" -t "$depth" --dump-vcd "$work/${engine}_bmc.vcd" "$smt2" 2>"$work/${engine}_bmc.err"; then
...
if yosys-smtbmc --unroll -s "$solver" -i -t "$depth" "$smt2" 2>"$work/${engine}_ind.err"; then
