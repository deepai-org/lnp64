...
cat >"$debug_line_c" <<'C'
extern int debug_line_callee(int);
__attribute__((noinline)) int debug_line_probe(int x) {
  int y = x + 7;
  return debug_line_callee(y * 3);
}
C
...
libc_test_fcntl_obj="$build_dir/libc-test-fcntl-clang-smoke.o"
"$clang" --target=lnp64-unknown-none -ffreestanding -fno-builtin -fno-pic -fno-jump-tables \
  -fno-unwind-tables -fno-asynchronous-unwind-tables -I toolchain/include \
  -I third_party/libc-test/functional \
  -c third_party/libc-test/functional/fcntl.c \
  -o "$libc_test_fcntl_obj"
test -s "$libc_test_fcntl_obj"
libc_test_fcntl_dump="$build_dir/libc-test-fcntl-clang-smoke.dump"
"$llvm_objdump" -d --triple=lnp64-unknown-none "$libc_test_fcntl_obj" \
  >"$libc_test_fcntl_dump"
grep -q 'call ' "$libc_test_fcntl_dump"
printf 'real LLVM LNP64 clang libc-test fcntl object smoke passed: %s\n' \
  "$libc_test_fcntl_obj"
...
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  ...
  "$libc_string_impl_obj"
test -s "$libc_test_fcntl_elf"
printf 'real LLVM LNP64 lld libc-test fcntl link smoke passed: %s\n' \
  "$libc_test_fcntl_elf"
...
sbase_tee_elf="$build_dir/lnp64-sbase-tee-linked.elf"
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  -o "$sbase_tee_elf" "$crt0_obj" "$build_dir/sbase-tee-clang-smoke.o" \
  "$build_dir/sbase-libutil-writeall-clang-smoke.o" \
  "$sbase_head_support_impl_obj" "$libc_alloc_impl_obj" "$libc_fd_impl_obj" \
  "$libc_signal_impl_obj" "$libc_string_impl_obj" "$libc_errno_impl_obj" \
  "$libc_process_impl_obj"
test -s "$sbase_tee_elf"
...
sbase_cut_elf="$build_dir/lnp64-sbase-cut-linked.elf"
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  -o "$sbase_cut_elf" "$crt0_obj" "$build_dir/sbase-cut-clang-smoke.o" \
  "$sbase_head_support_impl_obj" "$libc_alloc_impl_obj" \
  "$libc_fd_impl_obj" "$libc_convert_impl_obj" "$libc_string_impl_obj" \
  "$libc_errno_impl_obj" "$libc_process_impl_obj"
test -s "$sbase_cut_elf"
printf 'real LLVM LNP64 lld sbase cut link smoke passed: %s\n' \
  "$sbase_cut_elf"

sbase_tr_elf="$build_dir/lnp64-sbase-tr-linked.elf"
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  -o "$sbase_tr_elf" "$crt0_obj" "$build_dir/sbase-tr-clang-smoke.o" \
  "$sbase_head_support_impl_obj" "$libc_alloc_impl_obj" \
  "$libc_fd_impl_obj" "$libc_string_impl_obj" "$libc_errno_impl_obj" \
  "$libc_process_impl_obj"
test -s "$sbase_tr_elf"
printf 'real LLVM LNP64 lld sbase tr link smoke passed: %s\n' \
  "$sbase_tr_elf"

sbase_sort_elf="$build_dir/lnp64-sbase-sort-linked.elf"
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  -o "$sbase_sort_elf" "$crt0_obj" "$build_dir/sbase-sort-clang-smoke.o" \
  "$build_dir/sbase-libutil-getlines-clang-smoke.o" \
  "$build_dir/sbase-libutil-linecmp-clang-smoke.o" \
  "$sbase_head_support_impl_obj" "$libc_sort_impl_obj" \
  "$libc_convert_impl_obj" "$libc_alloc_impl_obj" "$libc_fd_impl_obj" \
  "$libc_string_impl_obj" "$libc_errno_impl_obj" "$libc_process_impl_obj"
test -s "$sbase_sort_elf"
...
sbase_sed_elf="$build_dir/lnp64-sbase-sed-linked.elf"
"$lld" -flavor gnu -static -m elf64lnp64 -T "$linker_script" \
  -o "$sbase_sed_elf" "$crt0_obj" "$build_dir/sbase-sed-clang-smoke.o" \
  "$sbase_head_support_impl_obj" "$libc_alloc_impl_obj" \
  "$libc_fd_impl_obj" "$libc_string_impl_obj" "$libc_convert_impl_obj" \
  "$libc_errno_impl_obj" "$libc_process_impl_obj"
test -s "$sbase_sed_elf"
printf 'real LLVM LNP64 lld sbase sed link smoke passed: %s\n' \
  "$sbase_sed_elf"
