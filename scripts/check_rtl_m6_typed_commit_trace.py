#!/usr/bin/env python3
"""Check the M6 RTL typed service commit trace.

Follows the M1/M7 typed-trace pattern for the SG-MEM slice. The lnp64_m6_service
engine emits a schema-owned packed commit (lnp64_m6_service_commit_t) and state
projection (lnp64_m6_state_projection_t) per VMA transition: mmap, permitted
...
DEFAULT_M6_TRACE_LOG = Path("/tmp/lnp64_rtl_m6_typed_commit.log")

COMMIT_NAME = "m6_service_commit"
STATE_NAME = "m6_state_projection"
COMMIT_BITS_NAME = "m6_service_commit_bits"
STATE_BITS_NAME = "m6_state_projection_bits"
COMMIT_RECORD = "lnp64_m6_service_commit_t"
STATE_RECORD = "lnp64_m6_state_projection_t"
OP_ENUM = "lnp64_m6_service_op_e"

WITNESS_SCHEMA = "lnp64_m6_service_refinement_witness_v1"
...
def fail(message: str) -> None:
    raise SystemExit(f"M6 typed commit check failed: {message}")
...
def run_m6_gate() -> str:
    log_path = Path(os.environ.get("LNP64_M6_TYPED_COMMIT_LOG", DEFAULT_M6_TRACE_LOG))
    if os.environ.get("LNP64_M6_TYPED_COMMIT_USE_EXISTING") == "1":
        try:
            return log_path.read_text(encoding="utf-8")
        except OSError as exc:
            fail(f"missing existing M6 typed commit log {log_path}: {exc}")
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        fail(f"scripts/run_rtl_m6.sh exited with {proc.returncode}")
    try:
...
def expected_sequence(ops: dict[str, int]) -> list[int]:
    return [
        ops["LNP64_M6_COMMIT_ENVELOPE"],
        ops["LNP64_M6_COMMIT_NS_DISPATCH"],
        ops["LNP64_M6_COMMIT_SERVICE_REQUEST"],
        ops["LNP64_M6_COMMIT_CAP_RETURN"],
        ops["LNP64_M6_COMMIT_SERVICE_CANCEL"],
        ops["LNP64_M6_COMMIT_STALE_SERVICE"],
        ops["LNP64_M6_COMMIT_CRASH_COMPLETION"],
    ]
...
def check_transition_trace(commits: list[dict], states: list[dict], ops: dict[str, int]) -> None:
    ...
    if actual != expected_sequence(ops):
        fail(f"M6 typed commit sequence drifted: {actual} != {expected_sequence(ops)}")
    by_op = {v: k for k, v in ops.items()}
    for index, (commit, state) in enumerate(zip(commits, states, strict=True)):
        op = require_int(commit, "op")
        # A service-returned capability can never carry rights the caller did
        # not request -- no authority amplification across the service boundary.
        if (require_int(commit, "returned_rights") & ~require_int(commit, "requested_rights")) != 0:
            fail(f"M6 commit {index} ({by_op.get(op)}) returned amplified rights")
        if require_int(commit, "op") != require_int(state, "op") or require_int(commit, "status") != require_int(state, "status"):
            fail(f"M6 commit {index} op/status drifted from state projection")
        if op == ops["LNP64_M6_COMMIT_ENVELOPE"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "envelope_validated") != 1:
                fail(f"M6 envelope commit {index} did not validate the envelope")
        elif op == ops["LNP64_M6_COMMIT_NS_DISPATCH"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "namespace_dispatched") != 1:
                fail(f"M6 ns-dispatch commit {index} did not dispatch the namespace request")
        elif op == ops["LNP64_M6_COMMIT_SERVICE_REQUEST"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "service_continuation_created") != 1:
                fail(f"M6 service-request commit {index} did not create the continuation")
        elif op == ops["LNP64_M6_COMMIT_CAP_RETURN"]:
            if require_int(commit, "status") != ERR_OK or require_int(state, "cap_return_installed") != 1:
                fail(f"M6 cap-return commit {index} did not install the returned cap")
            if require_int(state, "returned_cap_narrowed") != 1:
                fail(f"M6 cap-return commit {index} did not narrow the returned cap")
        elif op == ops["LNP64_M6_COMMIT_SERVICE_CANCEL"]:
            if require_int(commit, "status") != ERR_ECANCELED or require_int(state, "cancel_terminal") != 1:
                fail(f"M6 service-cancel commit {index} did not reach the terminal cancel")
        elif op == ops["LNP64_M6_COMMIT_STALE_SERVICE"]:
            if require_int(commit, "status") != ERR_EREVOKED or require_int(state, "stale_service_rejected") != 1:
                fail(f"M6 stale-service commit {index} did not reject the stale service generation")
        elif op == ops["LNP64_M6_COMMIT_CRASH_COMPLETION"]:
            if require_int(commit, "status") != ERR_EIO or require_int(state, "crash_completed") != 1:
                fail(f"M6 crash-completion commit {index} did not complete the crashed request")
        else:
            fail(f"M6 commit {index} has unknown op {op}")
...
def main() -> int:
    ...
    output = run_m6_gate()
    commits = parse_json_records(output, "TTRACE_M6 ", COMMIT_NAME, commit_fields)
    commit_bits = parse_bit_records(output, "TTRACE_M6_BITS ", COMMIT_BITS_NAME, sum(commit_widths))
    states = parse_json_records(output, "TTRACE_M6_STATE ", STATE_NAME, state_fields)
    state_bits = parse_bit_records(output, "TTRACE_M6_STATE_BITS ", STATE_BITS_NAME, sum(state_widths))
    check_bits(commits, commit_bits, commit_fields, commit_widths, "M6 typed commit")
    check_bits(states, state_bits, state_fields, state_widths, "M6 state projection")
    if len(commits) != len(states):
        fail(f"M6 commit count {len(commits)} != state projection count {len(states)}")
    check_transition_trace(commits, states, ops)

    witness_out = os.environ.get("LNP64_RTL_M6_WITNESS_OUT")
    if witness_out:
    ...
    print("rtl m6 typed commit trace ok")
    return 0
