...
build_dir="${TMPDIR:-/tmp}/lnp64_rtl_m8_obj"
seeds="${LNP64_COSIM_SEEDS:-0}"
rm -rf "$build_dir"

verilator --lint-only "${common_flags[@]}" "${rtl_files[@]}"
verilator --binary --Mdir "$build_dir" "${common_flags[@]}" "${rtl_files[@]}" >/tmp/lnp64_rtl_m8_build.log
...
for seed in $seeds; do
  ...
  "$build_dir/Vlnp64_m8_tb" "+seed=$seed" | tee "$rtl_log"
