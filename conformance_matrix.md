...
| musl libc-test subset | partial / passing focused checks | `third_party/libc-test/functional/{argv.c,basename.c,clock_gettime.c,dirname.c,env.c,string.c,string_memmem.c,string_strchr.c,string_strcspn.c,string_strstr.c}`, `scripts/run_libc_test.sh` | Expand beyond the initial path/string subset. `COMPAT-PKG-005` tracks harness and libc gaps. |
...
| `COMPAT-PKG-005` | musl libc-test subset | Upstream libc-test `argv`, `basename`, `clock_gettime`, `dirname`, `env`, `string`, `string_memmem`, `string_strchr`, `string_strcspn`, and `string_strstr` pass with a minimal LNP64 harness. Probed blockers include `basename`/`dirname` now-fixed null/all-slash behavior, high-byte C string escapes now-fixed by byte-exact string emission, file-scope static byte arrays now-fixed for `string_strchr`, mutable `environ` now covered by `env`, variadic macros now covered by `argv`, `memmem` byte-search semantics now covered by `string_memmem`, bounded `strncpy`/`strncat`, `strtok`, and `strlcat` return semantics now covered by `string`, and `memcpy`/`memset` function-pointer declarations now fixed by callable runtime labels. Full upstream `string_memcpy`/`string_memset` exhaustive alignment loops remain too slow for the default gate; `udiv` still exposes anonymous static aggregate field layout, and broader scanf/regex/wide/dynamic-linking tests remain open. | Add bounded memory-function gates or optimize emulator throughput for the full upstream memory stress tests; next practical targets include the `udiv` aggregate-field blocker. |
