...
| `COMPAT-PKG-005` | musl libc-test subset | Upstream libc-test `argv`, `basename`, `clock_gettime`, `dirname`, `env`, `search_lsearch`, `string`, `string_memmem`, `string_strchr`, `string_strcspn`, `string_strstr`, and `udiv` pass with a minimal LNP64 harness. Probed blockers include `basename`/`dirname` now-fixed null/all-slash behavior, high-byte C string escapes now-fixed by byte-exact string emission, file-scope static byte arrays now-fixed for `string_strchr`, file-scope two-dimensional byte arrays now covered by `search_lsearch`, mutable `environ` now covered by `env`, variadic macros now covered by `argv`, `memmem` byte-search semantics now covered by `string_memmem`, `lfind`/`lsearch` fixed-width search semantics now covered by `search_lsearch`, bounded `strncpy`/`strncat`, `strtok`, and `strlcat` return semantics now covered by `string`, anonymous static aggregate arrays now covered by `udiv`, and `memcpy`/`memset`/`strcmp` function-pointer declarations now fixed by callable runtime labels. Full upstream `string_memcpy`/`string_memset` exhaustive alignment loops remain too slow for the default gate; broader scanf/regex/wide/dynamic-linking tests remain open. | Add bounded memory-function gates or optimize emulator throughput for the full upstream memory stress tests; next practical targets include small scalar/time/path libc-test files. |
