...
| musl libc-test subset | partial / passing focused checks | `third_party/libc-test/functional/{basename.c,dirname.c,string_strcspn.c,string_strstr.c}`, `scripts/run_libc_test.sh` | Expand beyond the initial path/string subset. `COMPAT-PKG-005` tracks harness and libc gaps. |
...
| `COMPAT-PKG-005` | musl libc-test subset | Upstream libc-test `basename`, `dirname`, `string_strcspn`, and `string_strstr` pass with a minimal LNP64 harness. Probed blockers include the upstream variadic macro harness, `basename`/`dirname` now-fixed null/all-slash behavior, high-byte C string escapes now-fixed by byte-exact string emission, `memmem`, and broader scanf/regex/wide/dynamic-linking tests. | Add more upstream functional tests as C preprocessor and libc coverage grows; next practical targets are `strchr`, `memcpy`/`memset`, and simple environment tests. |
