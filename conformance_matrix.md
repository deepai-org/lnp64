...
| `COMPAT-PKG-005` | musl libc-test subset | Upstream libc-test `basename`, `dirname`, `env`, `string_strchr`, `string_strcspn`, and `string_strstr` pass with a minimal LNP64 harness. Probed blockers include the upstream variadic macro harness, `basename`/`dirname` now-fixed null/all-slash behavior, high-byte C string escapes now-fixed by byte-exact string emission, file-scope static byte arrays now-fixed for `string_strchr`, mutable `environ` now covered by `env`, and `memcpy`/`memset` function-pointer declarations now fixed by callable runtime labels. Full upstream `string_memcpy`/`string_memset` exhaustive alignment loops remain too slow for the default gate; `memmem` and broader scanf/regex/wide/dynamic-linking tests remain open. | Add bounded memory-function gates or optimize emulator throughput for the full upstream memory stress tests; next practical targets include `strrchr`/`memchr`. |
