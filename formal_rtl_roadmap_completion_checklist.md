...
| Top-level program corpus from existing assembly and compiler demos | `tests/rtl/top_level_program_manifest.json` tracks every `demos/*.s` file plus active LLVM clang/linked C entries as feature-gated future `lnp64_top` Verilator tests | `scripts/check_rtl_top_level_program_manifest.py` |
