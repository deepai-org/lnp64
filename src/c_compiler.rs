...
impl CodeGen {
    ...
    fn emit_fgets(&mut self, buf: usize, size: usize, stream: usize) -> Result<usize, String> {
        ...
        let read_memstream = self.new_label("fgets_read_memstream");
        let memstream_eof = self.new_label("fgets_memstream_eof");
        let after_read = self.new_label("fgets_after_read");
        ...
        self.text.push("  LI r20, -2".to_string());
        self.text.push(format!("  CMP r{fd}, r20"));
        self.text.push(format!("  BEQ {read_memstream}"));
        self.emit_read_fd_dispatch(fd, ptr, count, Some(count))?;
        self.text.push(format!("  JMP {after_read}"));
        self.text.push(format!("{read_memstream}:"));
        self.text.push("  LD r20, c_memstream_pos".to_string());
        self.text.push("  LD r21, c_memstream_len".to_string());
        self.text.push("  CMP r20, r21".to_string());
        self.text.push(format!("  BGE {memstream_eof}"));
        self.text.push("  LD r22, c_memstream_ptr".to_string());
        self.text.push("  ADD r22, r22, r20".to_string());
        self.text.push(format!("  LD.B r{ch}, [r22, 0]"));
        self.text.push(format!("  CMP r{ch}, r0"));
        self.text.push(format!("  BEQ {memstream_eof}"));
        self.text.push(format!("  ST.B [r{ptr}, 0], r{ch}"));
        self.text.push(format!("  LI r{count}, 1"));
        self.text.push(format!("  ADD r20, r20, r{count}"));
        self.text.push("  ST c_memstream_pos, r20".to_string());
        self.text.push(format!("  JMP {after_read}"));
        self.text.push(format!("{memstream_eof}:"));
        self.text.push(format!("  LI r{count}, 0"));
        self.text.push(format!("{after_read}:"));
