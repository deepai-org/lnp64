...
fn encode_flat_exec_instr(
    ...
    match instr {
        ...
        Instr::AwaitExDyn(result, fd_reg, argblock) => {
            Ok(vec![enc_rrr(0x72, *result, *fd_reg, *argblock)])
        }
        Instr::WaitableProbe(result, fd, mask) => {
            Ok(vec![enc_rrr(0x6f, *result, Reg(fd.0), *mask)])
        }
        Instr::WaitableProbeDyn(result, fd_reg, mask) => {
            Ok(vec![enc_rrr(0x70, *result, *fd_reg, *mask)])
        }
