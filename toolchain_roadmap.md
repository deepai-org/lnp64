...
`ST_B`/`ST_H`/`ST_W`.
The first private native stream shim lowerings recognize `__lnp_pull` and
`__lnp_push` and emit native `PULL`/`PUSH` operations directly, treating the C
ABI `lnp64_cap_t` as a GPR capability handle. The remaining `__lnp_*` shims
still need backend nodes or runtime call fallbacks.
