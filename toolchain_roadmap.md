...
path helper, search helper, `calloc`/`realloc`, `read`, `errno`, `exit`, and
real Clang demo link/run smokes now link against Clang-built tiny C
...
`toolchain/liblnp64_alloc_min.c`, `toolchain/liblnp64_fd_min.c`,
`toolchain/liblnp64_process_min.c`, `toolchain/liblnp64_errno_min.c`, and
`toolchain/liblnp64_poll_min.c` are the checked source files for those tiny C
shim objects.
