...
real target contracts, the first LLVM/Clang path, libc/runtime shims, software
loader/exec-plan work, NetBSD personality layering, and conformance gates. The
Rust test
...
`toolchain/lnp64_llvm_bootstrap.manifest` names the tested Clang-built program
set for the real toolchain path: demos, package smokes, sbase commands,
userland fixtures, NetBSD personality children, and simple libc. The broader
libc/runtime replacement remains partial until the smoke-only shim is replaced
by the production crt/libc runtime.
...
`tested`; all current rows in the manifest are tested. The fast post-build
inner loop is `scripts/run_real_llvm_bootstrap_smokes.sh`, which rebuilds the
minimal hello, arithmetic, memory, and calls programs with the existing real
LLVM build, links them with the packaged sysroot, then runs `elf-plan` and
`run-elf`.
...
`zlib` checksums, `natsort`, `jsmn`, string-only `inih`, and `cwalk` package smokes over real package
...
`calloc`/`realloc`, minilibc `read` smokes, and the first normal-C
`userland/init_clang.c`, `userland/lnpsh_clang.c`,
`userland/spawn_task_clang.c`, and `userland/ucat_clang.c` commands under
namespace-rooted or native `run-elf` probes. The gate assembles the checked
...
compiler as special cases. `toolchain/lnp64_intrinsic_lowering.manifest` now
...
Frame lowering reserves no scratch register: v2's `ADDI` carries a full
signed-32 immediate, so stack adjustments are a single `addi r31, r31, ±size`
and `r30` is an ordinary allocatable GPR. Non-leaf frames spill/restore the
return address `r1` (ra) with ordinary `SD`/`LD` and emit the v0 minimum DWARF
CFI for the CFA offset and saved `ra` slot. Varargs, stack
...
6. Pin native capability-call intrinsics.
   - `__lnp_gate_call`/`__lnp_call` lower to
     `GATE_CALL result, gate_fd, arg0, arg1`.
   - synchronous calls return nonnegative success values or negative
     architectural errors in the encoded result register.
   - asynchronous calls return operation ids/tokens and complete through
     event/counter/queue objects consumed with `AWAIT_EX`/`PULL`.
   - `__lnp_gate_return` lowers to `GATE_RETURN result, value0, value1`.
   - No general syscall instruction is introduced for native object/domain/cap
     operations; POSIX/Linux syscall compatibility remains a runtime/personality
     layer over native primitives.

7. Build the software loader path.
...
2. Private libc/native shim layer over `__lnp_*` intrinsics.
3. Minimal crt objects or documented compiler-emitted startup transition pinned
   ...
   `toolchain/lnp64_sysroot.manifest` and
   `scripts/package_lnp64_sysroot.sh` now package that crt object, the checked
   linker script, target headers, and the current Clang-built libc shim objects
   into `target/lnp64-sysroot`, then link and run a sysroot-backed smoke ELF.
...
5. Keep moving the NetBSD personality system gate from fixed-record fixtures
   ...
   Clang/lld/run-elf init/shell root plus child ELF set.
...
- The emulator runs the program through the software loader.
...
- `netbsd_personality_clang`: the first NetBSD personality slice built with
  Clang/lld and run through the ELF loader.
...
loader. Full runtime replacement remains partial while broader libc, package,
and NetBSD-derived coverage continues moving onto those gates.

The tested command shapes are pinned in
...
pinned in `toolchain/lnp64_clang_driver.manifest`. The packaged sysroot layout
and sysroot-backed static-link/run-elf smoke are pinned in
`toolchain/lnp64_sysroot.manifest` and generated by
`scripts/package_lnp64_sysroot.sh`. The narrow post-build bootstrap gate is
`scripts/run_real_llvm_bootstrap_smokes.sh`: it verifies the minimal
hello/arithmetic/memory/calls path through real Clang, lld, `elf-plan`, and
`run-elf` without rebuilding LLVM. The implementation file
...
The run-elf path is tested: linked hello, factorial, allocator, Fibonacci, PCR,
cat/openat, JSON parser, rot13, producer/consumer, parallel hash, sqlite lite,
and ping-pong smokes now run through real Clang/lld output.
...
The transition is intentionally layered so real Clang/lld and the software
loader define C execution:
...
| Minimal LLVM/Clang path | `toolchain/lnp64_llvm_bootstrap.manifest` pins tested hello, arithmetic, memory, calls, simple-libc, PCR, cat/openat, json parser, rot13, producer/consumer, parallel hash, sqlite lite, ping-pong, sbase command, netcat, httpd, userland init/lnpsh/spawn-task/ucat, and NetBSD personality child gates; `toolchain/lnp64_llvm_gates.manifest`, `scripts/run_llvm_bootstrap_gates.sh --dry-run`, and `scripts/run_real_llvm_bootstrap_smokes.sh` pin tested Clang/lld/loader command shapes for hello/arithmetic/memory/calls, crt0, static link, exec-plan inspection, and run-elf; `Dockerfile.llvm`, `scripts/run_real_llvm_tblgen_docker.sh`, and `scripts/run_real_llvm_lnp64_docker.sh` pin the real LLVM TableGen, Clang scalar, hello object, factorial object, allocator object, Fibonacci calls object, PCR demo object, cat demo object, json parser demo object, rot13 demo object, producer/consumer demo object, parallel hash demo object, sqlite lite demo object, ping-pong demo object, netcat/httpd demo object/static links plus self-test run-elf coverage, zlib checksum, natsort, jsmn, inih, and cwalk package object/link/run-elf smokes, sbase 27-command and libutil object smokes, sbase yes static-link/exec-plan coverage, sbase echo, path-command, cat, wc, head, cmp, cksum, uniq, tail, tee, cp, cut, tr, sort, fixed-string grep, no-regex sed, mkdir, ln, chmod, chown, touch, ls, find, mv, and rm link/run-elf smokes, userland init/lnpsh/spawn-task/ucat link/run-elf smokes, NetBSD personality clang link/run-elf smoke, Clang-built string/memory/ctype, numeric conversion, path helper, search helper, sort helper, allocation, fd, process, errno, futex, poll/select/epoll/kqueue, signal, and socket libc objects, legacy smoke libc assembly, codegen, MC, disassembly, GET_PCR and OPEN_AT intrinsics, lld static-link, and run-elf smoke gates; `toolchain/lnp64_toy_compiler_retirement.manifest` records the custom Rust C frontend as removed and limits the remaining legacy assembler/flat-exec path to assembly and RTL smoke generation while keeping C/package/userland coverage on real Clang/lld/run-elf; `toolchain/lnp64_run_elf.manifest` pins the execution boundary between loader commit and real ELF text execution; `toolchain/lnp64_clang_driver.manifest` pins the driver defaults and `third_party/sbase/fs.h` pins the sbase recursive-file frontend declarations and `toolchain/include/{assert,ctype,dirent,errno,fcntl,fnmatch,grp,inttypes,libgen,limits,poll,pthread,pwd,regex,search,semaphore,signal,stdarg,stddef,stdint,stdio,stdlib,string,strings,netinet/in,sys/auxv,sys/epoll,sys/event,sys/mman,sys/select,sys/socket,sys/stat,sys/sysmacros,sys/timerfd,sys/types,sys/wait,time,unistd,utime}.h` pins the first target C header root; `toolchain/lnp64_sysroot.manifest` and `scripts/package_lnp64_sysroot.sh` pin the packaged crt/header/libc-shim sysroot layout plus a sysroot-backed static-link/run-elf smoke; `toolchain/lnp64_llvm_filemap.manifest` pins the llvm-project source surface; `toolchain/lnp64_static.ld` pins the first lld static layout; `toolchain/crt0_lnp64.s` pins the first crt0 startup stub; `toolchain/liblnp64_min.s` pins the legacy assembler libc opcode smoke; `toolchain/liblnp64_string_min.c`, `toolchain/liblnp64_convert_min.c`, `toolchain/liblnp64_path_min.c`, `toolchain/liblnp64_search_min.c`, `toolchain/liblnp64_sort_min.c`, `toolchain/liblnp64_alloc_min.c`, `toolchain/liblnp64_fd_min.c`, `toolchain/liblnp64_process_min.c`, `toolchain/liblnp64_errno_min.c`, `toolchain/liblnp64_time_min.c`, `toolchain/liblnp64_vma_min.c`, `toolchain/liblnp64_poll_min.c`, `toolchain/liblnp64_signal_min.c`, `toolchain/liblnp64_socket_min.c`, `toolchain/liblnp64_sbase_min.c`, `toolchain/liblnp64_sbase_fs_min.c`, `toolchain/liblnp64_sbase_recurse_min.c`, `toolchain/liblnp64_sbase_move_min.c`, `toolchain/liblnp64_sbase_time_min.c`, `toolchain/liblnp64_sbase_ls_min.c`, `toolchain/liblnp64_sbase_find_min.c`, `toolchain/liblnp64_sbase_accounts_min.c`, `toolchain/liblnp64_sbase_wc_min.c`, and `toolchain/liblnp64_sbase_head_min.c` pin the checked C shim sources; `toolchain/lnp64_intrinsics.h` pins the private C shim header; `toolchain/lnp64_mc_encoding.manifest` pins the first MC encoding and relocation hooks. | `llvm_bootstrap_manifest_names_first_clang_gate`, `llvm_gate_manifest_pins_clang_lld_loader_commands`, `run_elf_manifest_records_execution_boundary`, `toy_compiler_retirement_manifest_limits_custom_frontend_scope`, `clang_driver_manifest_matches_llvm_gates`, `sysroot_manifest_records_packaged_crt_layout`, `llvm_filemap_manifest_names_backend_source_surface`, `mc_encoding_manifest_covers_initial_backend_opcodes`, `crt0_startup_stub_matches_crt_contract`, and `intrinsic_header_matches_intrinsic_manifest` |
...
The `minimal_llvm_clang_path` transition row remains partial even though the
bootstrap manifest rows are tested. It is not complete until the
Clang/lld/software-loader path covers the libc/runtime replacement without the
smoke-only libc shim.
