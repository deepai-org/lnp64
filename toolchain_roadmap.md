...
| Software loader and exec plan | `src/loader.rs`, `src/emulator.rs`, `object_format.md`, and `toolchain/lnp64_exec_plan.manifest` define the initial ELF64 parser, encoded exec-plan records, and emulator-side descriptor validation for the bounded `EXEC` commit boundary. | `exec_plan_manifest_matches_loader_boundary_contract` plus `emulator_accepts_loader_encoded_exec_descriptor_shape` |
