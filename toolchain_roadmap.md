...
libc/runtime is ready. The string/memory, `calloc`/`realloc`, `read`, and
`exit` simple-libc smokes now link against Clang-built tiny C implementations
instead of this assembler shim, keeping those slices on the real C toolchain
path.
