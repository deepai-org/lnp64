.text
# Pipe push/pull over the unified IPC verbs (F1-step-2 migration of the old
# WRITE_FD_DYN/READ_FD_DYN form; 0x3b/0x3c are now retired). send (0x83) and
# recv (0x84) decode to the WRITE_FD/READ_FD datapath in the RTL with
# operand-sourcing as the only fork: the endpoint handle is in rs1 and a pointer
# to the frozen msg descriptor ([0]=bytes_ptr, [8]=bytes_len, [16]=caps_ptr,
# [24]=caps_len) is in rs2. The byte-fd ABI returns the transfer count in r2.
  LI r29, -1
  LI r10, 0
