...
**A process is not a separate object kind — it *is* a Resource Domain** that holds an address-space capability plus one or more threads (the schedulable leaves). "Process", "container", and "VM" differ only by which slots a domain populates; there is no `process`-vs-`container` type tag and no leaf-vs-interior variant (those words are descriptive of populated slots only). `fork`/`exec`/`clone` fall out of capability-sharing: `fork` spawns a child domain that COW-inherits the parent's address-space capability and FDR/endpoint table; `exec` replaces the image a domain holds; the `clone()` spectrum is just *which* capabilities are shared with the child. PID/PPID/pgrp/session are domain-keyed **names (data), never authority** — `kill(pid,…)` resolves the name in hardware *and* checks a capability to the target. This unification (the object model and its `send`/`recv`/`gate_call`/`wait` endpoint verbs) is specified in [`unified_object_model.md`](unified_object_model.md); the domain delegate/budget mechanism is M14-proven.
